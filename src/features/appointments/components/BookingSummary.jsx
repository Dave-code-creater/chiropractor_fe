...
import { CalendarDays, Clock, MapPin, User, FileText, Stethoscope } from "lucide-react";
...
export default function BookingSummary({ bookingData, doctors = [] }) {
    ...
    const getAppointmentTypeLabel = (type) => {
        ...
        return types[type] || "Standard Appointment";
    };
    ...
    const isEmpty = !bookingData.doctor && !bookingData.location && !bookingData.date && !bookingData.time;

    if (isEmpty) {
        return (
            <div className="text-center py-8 text-muted-foreground">
                <CalendarDays className="h-12 w-12 mx-auto mb-3 opacity-50" />
                <p className="text-sm">Your appointment details will appear here as you make selections.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
                ...
                <>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">{getLocationLabel(bookingData.location)}</p>
                            <p className="text-xs text-muted-foreground mt-1">1385 W Alameda Ave, Denver, CO 80223</p>
                        </div>
                    </div>
                </>
            )}
                        ...
                        {bookingData.time && (
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">{formatTime(bookingData.time)}</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <Badge variant="secondary" className="mt-1">
                                {getAppointmentTypeLabel(bookingData.type)}
                            </Badge>
                        </div>
                    </div>
                </>
                ...
                <>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">{bookingData.reason}</p>
                        </div>
                    </div>
                </>
            )}
            ...
            {/* Estimated Duration */}
            {(selectedDoctor || bookingData.location || bookingData.date || bookingData.time) && (
                <>
                    <Separator />
                    <div className="flex items-start gap-3">
                        <Clock className="h-5 w-5 text-primary mt-0.5 flex-shrink-0" />
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">30 minutes (estimated)</p>
                        </div>
                    </div>
                </>
            )}
            ...
            {(selectedDoctor || bookingData.location || bookingData.date || bookingData.time) && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-4">
                    <h4 className="text-sm font-medium text-blue-900 mb-2">Important Reminders:</h4>
                    <ul className="text-xs text-blue-800 space-y-1">
                        ...
                        {selectedDoctor && (
                            <li>• Dr. {selectedDoctor.lastName} specializes in {selectedDoctor.specializations?.primary || selectedDoctor.specialization}</li>
                        )}
                    </ul>
                </div>
            )}
