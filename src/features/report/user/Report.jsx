...
const ReportCard = React.memo(({ report, onEdit, onDuplicate, onDelete }) => {
    ...
    const handleDuplicate = useCallback(() => onDuplicate(report.id), [onDuplicate, report.id]);
    const handleDelete = useCallback(() => onDelete(report.id), [onDelete, report.id]);
                    ...
                    {report.completionPercentage !== undefined && (
                        <div className="space-y-2">
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="text-gray-900">{report.completionPercentage}%</span>
                            </div>
...
const ErrorState = React.memo(({ error, onRetry }) => (
    <div className="text-center py-12">
        ...
        <p className="text-gray-600 mb-4">
            {error?.data?.message || error?.message || 'Something went wrong while loading your reports'}
        </p>
...
const EmptyState = React.memo(({ searchTerm, selectedCategory, onCreateReport }) => (
    <div className="text-center py-12">
        <FileTextIcon className="h-12 w-12 text-gray-300 mx-auto mb-4" />
        <h3 className="text-lg font-medium text-gray-900 mb-2">
            {searchTerm || selectedCategory !== 'all' ? 'No reports found' : 'No reports yet'}
        </h3>
        <p className="text-gray-600 mb-4">
...
export default function Report() {
    ...
    const handleTemplateSelect = useCallback(async (template) => {
        ...
        try {
            ...
            const newReportData = {
                ...
                template_type: template.folder?.toLowerCase().includes('consultation') ? 'consultation' : 
                              template.folder?.toLowerCase().includes('follow') ? 'follow-up' : 'assessment',
                status: 'draft',
            ...
            const newReport = {
                ...
                category: template.folder?.toLowerCase().includes('consultation') ? 'consultation' : 
                         template.folder?.toLowerCase().includes('follow') ? 'follow-up' : 'assessment',
                status: 'draft',
    ...
    const handleSubmit = useCallback(async (reportId, data) => {
        console.log('Report submitted:', { reportId, data });
    ...
    const handleDuplicate = useCallback(async (reportId) => {
        const report = reports.find(r => r.id === reportId);
        if (report) {
            try {
    ...
    const filteredReports = useMemo(() => {
        ...
        return uniqueReports.filter(report => {
            const matchesSearch = report.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                report.templateData?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                                report.templateData?.folder?.toLowerCase().includes(searchTerm.toLowerCase());
            ...
            <div className="bg-white border-b border-gray-200 px-6 py-4">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-600">Manage patient documentation and forms</p>
                    </div>
                    <Button 
