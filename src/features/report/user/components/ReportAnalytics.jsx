...
const ReportAnalytics = ({ reports = [] }) => {
    const analytics = useMemo(() => {
        ...
        const reportsThisMonth = reports.filter(r => new Date(r.createdAt) >= thirtyDaysAgo).length;
        const reportsThisWeek = reports.filter(r => new Date(r.createdAt) >= sevenDaysAgo).length;
        ...
        const avgCompletionTime = completedReports > 0 ? 
            Math.round(reports
                    ...
                    <CardContent className="text-orange-700">
                        <ul className="space-y-2 text-sm">
                            {analytics.draftReports > 3 && (
                                <li>• You have {analytics.draftReports} draft reports. Consider completing them to improve your workflow.</li>
                            )}
                            {analytics.completionRate < 70 && (
                                <li>• Your completion rate is {analytics.completionRate}%. Try setting reminders to finish reports.</li>
                            )}
                            {analytics.avgCompletionTime > 7 && (
                                <li>• Reports take an average of {analytics.avgCompletionTime} days to complete. Consider breaking them into smaller tasks.</li>
                            )}
                        </ul>
                    </CardContent>
                </Card>
