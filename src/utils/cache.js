...
export const withCache = (
    ...
    return async (...args) => {
        ...
        if (cached) {
            console.log(`Cache hit for ${namespace}:${key}`);
            return cached;
        }

        // If not in cache, make API call
        console.log(`Cache miss for ${namespace}:${key}, fetching from API`);
        try {
...
export const cacheUtils = {
    ...
    blog: {
        getPosts: (params) => cacheManager.get(CACHE_NAMESPACES.BLOG, 'posts', params),
        setPosts: (data, params, ttl = CACHE_DURATION.MEDIUM) => 
            cacheManager.set(CACHE_NAMESPACES.BLOG, 'posts', data, params, ttl),
        getPost: (slug) => cacheManager.get(CACHE_NAMESPACES.BLOG, 'post', { slug }),
    ...
    appointments: {
        getList: (params) => cacheManager.get(CACHE_NAMESPACES.APPOINTMENTS, 'list', params),
        setList: (data, params, ttl = CACHE_DURATION.SHORT) => 
            cacheManager.set(CACHE_NAMESPACES.APPOINTMENTS, 'list', data, params, ttl),
        getAvailability: (doctorId, date) => 
            cacheManager.get(CACHE_NAMESPACES.APPOINTMENTS, 'availability', { doctorId, date }),
        setAvailability: (doctorId, date, data, ttl = CACHE_DURATION.SHORT) => 
            cacheManager.set(CACHE_NAMESPACES.APPOINTMENTS, 'availability', data, { doctorId, date }, ttl),
        clearAll: () => cacheManager.clearNamespace(CACHE_NAMESPACES.APPOINTMENTS),
    },

    // Doctors cache utilities
    doctors: {
        getList: (params) => cacheManager.get(CACHE_NAMESPACES.DOCTORS, 'list', params),
    ...
    user: {
        getProfile: (userId) => cacheManager.get(CACHE_NAMESPACES.USER, 'profile', { userId }),
